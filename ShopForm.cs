 ...
 namespace MusicStore
{
    public partial class Store : Form
    {
        ...
        public Store(string user, List<int> itemChecked)
        {
        ...
        public void executeSearch(string SearchText, string orderBy)
        {
            string query;
            if (orderBy == "Price")
                query = "select  i.ItemName, i.Description, i.Price, q.QualityName from inventory as w,item as i, category as c, category as p, quality as q  where(p.CategoryName like '%" + SearchText + "%'  or c.CategoryName like '%" + SearchText + "%' or  i.ItemName like '%" + SearchText + "%' ) and p.CategoryID = c.ParentID and c.CategoryID = i.CategoryID and q.QualityID = i.QualityID and w.ItemID = i.ItemID order by Price;";
            else if (orderBy == "Popularity")
                query = "select i.ItemName, i.Description, i.Price, q.QualityName from item as i, inventory as x, category as c, category as p, quality as q,  order_detail as w where(p.CategoryName like '%" + SearchText + "%'  or c.CategoryName like '%" + SearchText + "%' or i.ItemName like '%" + SearchText + "%' ) and (p.CategoryID = c.ParentID and c.CategoryID = i.CategoryID) and x.ItemID = i.ItemID and w.ItemID = i.ItemID and q.QualityID = i.QualityID order by w.Quantity desc;";
            else
                query = "select  i.ItemName, i.Description, i.Price, q.QualityName from inventory as x, item as i, category as c, category as p, quality as q where(p.CategoryName like '%" + SearchText + "%'  or c.CategoryName like '%" + SearchText + "%' or i.ItemName like '%"+SearchText+ "%') and p.CategoryID = c.ParentID and c.CategoryID = i.CategoryID and q.QualityID = i.QualityID and x.ItemID = i.ItemID ;";
        ...
        {
            //user is ready to checkout
            //first if username is "invalid" then user has to log in 
            if(username == "invalid")
            {
                LoginForm login = new LoginForm(itemIDsChecked);
            ...
            {
                Checkout checkoutpage = new Checkout(username, itemIDsChecked);
        ...
        private void accountButton_Click(object sender, EventArgs e)
        {
            ...
            }else
            {
                ...
                Account acct = new Account(username, id, itemIDsChecked);
                acct.FormClosed += new FormClosedEventHandler(form_closed);
                acct.Show();
                this.Hide();
