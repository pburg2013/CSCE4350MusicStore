...
namespace MusicStore
{
    public partial class SellPage : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            query = "INSERT INTO  inventory (ItemID, Quantity) VALUES (@itemID, @prodQuantity); ";
            try
            {
                myConnection.Open();
        ...
        public int add_item(int CatID, int QualID)
        {
            ...
            try
            {
                ...
                using (commandDatabase = new MySqlCommand(query, myConnection))
                {
                    ...
                    if (x > 0)
                    {
                        MessageBox.Show("Adding item to inventory...");
        ...
        {
            query = "select qualityID from quality where qualityName like '%" + prodQuality + "%'";
            myConnection.Open();
            commandDatabase = new MySqlCommand(query, myConnection);
            commandDatabase.CommandTimeout = 60;
            reader = commandDatabase.ExecuteReader();
